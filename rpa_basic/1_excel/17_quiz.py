...
# 퀴즈2 문제에 오류를 발견하여 모두 만점처리 하기로 하였습니다.
# 현재까지 작성된 최종 성적 데이터를 기준으로 아래와 같이 수정하시오.

# 1. 퀴즈2 점수를 10으로 수정
# 2. H열에 총점(SUM이용),I열에 성적 정보 추가
# -총점 90점이상A, 80점이상 B, 70점이상 C, 나머지 D
# 3. 출석이 5점 미만인 학생은 총점 상관없이 F
...
for idx, score in enumerate(scores, start=2): # 인덱스를 2번째 부터 시작
...
# 3. 출석이 5점 미만인 학생은 총점 상관없이 F
    ...
    if score[1] < 5:
        grade = "F"
    ws.cell(row=idx, column=9).value = grade # I 열에 성적 표시

...
# m = p.search("good care") # search  :주어진 문자열에서 일치하는게 있는지 확인
# print_match(m)
...
# 2. m = p.match("비교 할 문자열") : 주어진 문자열이 처음부터 일치하는지 확인
# 3. m = p.search("비교 할 문자열") :주어진 문자열에서 일치하는게 있는지 확인
# 4. lst = p.findall("비교 할 문자열")# findall : 일치하는 모든것을 "리스트" 형태로 반환
